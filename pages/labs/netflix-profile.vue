<template>
  <section class="page">
            ...
            <g id="Page-1" fill="none" fill-rule="evenodd">
              <g id="Adult-Child">
                <g id="profile2" transform="translate(13)">
                  <path d="M7 0C3.67 4 2 7.33 2 10c0 4 4 8 8 8s16-5 18-8-4 9-4 14 3 11 11 11c21 0 21.05-35 67-35H7z" id="hair" fill="#FCFDFF"/><path d="M143 121c18.23 0 33-14.77 33-33h-66c0 18.23 14.77 33 33 33z" id="right-eye-bottom" fill="#FCFDFF"/>
    ...
    <div class="manage" :class="{ active : button == 'Done' }">
      <span @click="manage()">{{ button }}</span>
    </div>
  </section>
</template>
  ...
  methods: {
    manage() {
      if (this.button != 'Done') {
        ...
        this.title = "Who's Watching?";
        this.button = 'Manage Profiles';
      }
...
.profile1 {
	#left-brow {
		...
		animation: raise $timing ease-in-out infinite;
	}
	#sunglasses {
		transform-origin: center 100%;
		animation: tilt $timing 250ms ease infinite;
	}
	#mustache {
		transform-origin: center 90%;
		animation: mustache $timing 0ms ease infinite;
	}
}
...
.profile2 {
	#left-pupil, #right-pupil {
		animation: side-to-side $timing ease-in-out infinite alternate;
	}
	#left-eye-top, #right-eye-top {
		transform-origin: center bottom;
		animation: wide-open $timing ease-in-out infinite alternate;
	}
	#bottom-beak, #gillard {
		transform-origin: center top;
		animation: chew $timing*2 ease-in-out infinite;
	}
	#hair {
...
// Profile #3
.profile3 svg {
  animation: bg-fade $timing ease infinite; 
  #smile {
    width: 100px;
  }
}
